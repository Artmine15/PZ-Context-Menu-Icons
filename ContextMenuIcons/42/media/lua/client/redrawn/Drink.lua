...
function ISInventoryPaneContextMenu.createMenu(player, isInPlayerInventory, items, x, y, origin)
    local context = GetInventoryCreateMenuContextIconized(originalCreateMenu, "ContextMenu_Drink", getRedrawnIconPath, "Drink.png", player, isInPlayerInventory, items, x, y, origin);

    local option = GetOptionFromContext("ContextMenu_Drink", context);
